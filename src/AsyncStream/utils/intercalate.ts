...
export function intercalate<A>(M: Monoid<A>) {
  /**
   * Takes a value to place between members of an {@link AsyncStream}.
   *
   * @param {A} middle The value to place.
   * @return {(fa: AsyncStream<A>) => A} A function that takes an async stream
  ...
  return function _intercalate(middle: A) {
     ...
     * @__PURE__
     */
    return function __intercalate(fa: AsyncStream<A>): Task<A> {
      return async function ___intercalate() {
        let curr = M.empty
        const interspersed = intersperse(middle)(fa)

        for await (const a of interspersed()) {
